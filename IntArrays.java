class Arrays {
    ...
    static int[] randomDigits(int length) {
        // Iniciar o vetor com o comprimento dado
        int[] array = new int[length];
        
        // Iniciar o objeto Random
        Random rand = new Random();
        
        // Preencher o vetor com dígitos aleatórios de 0 a 9
        int i = 0;
        while (i < length) {
            array[i] = rand.nextInt(10); // Gera um número aleatório entre 0 (inclusive) e 10 (exclusive)
            i++;
        }
    ...
    static double mean(int[] v) {
        ...
        // Usar um loop while para somar os elementos do vetor
        while (i < v.length) {
            total += v[i];
            i++;
        }
        ...
        // Calcular a média
        double average = (double) total / v.length;
    ...
    static int[] copy(int[] v, int length) {
        ...
        while (i < v.length && i < length) {
            newArray[i] = v[i];
            i++;
        }
        
        // Os espaços restantes no novo vetor serão automaticamente preenchidos com zeros
        // devido à inicialização padrão de arrays de inteiros em Java.
        
        return newArray;
    }

    static int[] copy(int[] v) {
        // Inicializar o novo vetor com o mesmo tamanho que o original
        int[] newArray = new int[v.length];
        
        // Inicializar a variável do índice
        int i = 0;
    ...
    static int max(int[] v) {
        ...
        if (v.length == 0) {
            return Integer.MIN_VALUE;  // Devolve o menor valor inteiro possível se o vetor estiver vazio
        }
    ...
    static int[] copyWithoutMiddleElement(int[] original) {
        int length = original.length;
        int newLength = (length % 2 == 1) ? length - 1 : length;
