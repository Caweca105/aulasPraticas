...
public class Sudoku {
    ...
    public Sudoku(String file, double difficulty) {
        ...
        this.boardImage = new ColorImage(9, 9); // Example dimensions, adjust as needed
    ...
    public void saveGame() {
        try (PrintWriter out = new PrintWriter("jogo1.sud")) {
            out.println(sudokuBoard.toString());
            out.close();
    ...
    public void loadGame(String filename) {
        // Loads the board state from a file and updates the current board
        int[][] loadedBoard = loadBoardLayout("jogo1.sudgame");
        for (int row = 0; row < 9; row++) {
            System.arraycopy(loadedBoard[row], 0, sudokuBoard.getBoard()[row], 0, 9);
        }
        updateBoardImage(); // Updates the visual representation of the board
